...
std::random_device rd;  // Use a hardware entropy source if available, otherwise seed
 ...
 */
void getImagePaths(const std::filesystem::path& path, std::vector<std::string>& destination) {
	if (std::filesystem::is_directory(path)) {
		for (auto& childPath: std::filesystem::directory_iterator(path)) {
...
std::vector<IntegralImage> computeIntegralsRand(std::vector<std::string>& paths, int baseResolution, int setSize) {
	...
	while (finalIntegrals.size() < setSize) {
		for (int i = 0; i < paths.size() && finalIntegrals.size() < setSize; i += 1, count += 1) {
			...
			std::uniform_int_distribution<> distX(0, image.width() - baseResolution);
			std::uniform_int_distribution<> distY(0, image.height() - baseResolution);
			int xCrop = distX(gen);
			int yCrop = distY(gen);
...
int main(int argc, char* argv[]) {
	...
	std::filesystem::path pathToPositives;
	std::filesystem::path pathToNegatives;
	std::filesystem::path pathToValidationNegatives;
	std::filesystem::path pathToValidationPositives;
	...
	for (int i = 1; i < argc; i += 1) {
		if (std::strcmp(argv[i], "--p") == 0) {
			pathToPositives = std::filesystem::path(argv[i + 1]);
		} else if (std::strcmp(argv[i], "--n") == 0) {
			pathToNegatives = std::filesystem::path(argv[i + 1]);
		} else if (std::strcmp(argv[i], "--vp") == 0) {
			pathToValidationPositives = std::filesystem::path(argv[i + 1]);
		} else if (std::strcmp(argv[i], "--vn") == 0) {
			pathToValidationNegatives = std::filesystem::path(argv[i + 1]);
